...
spikeTrigPhaseMap = zeros(Spikes.nSpikes,parameters.rows,parameters.cols);

for i=1:Spikes.nSpikes
    spikeTrigPhaseMap(i,:,:) = circ_mean(angle(LFP.xgp(:,:,Spikes.Clusters(i).spikeIndexLFP)),[],3);
end
% Plotting spike triggered phase maps
figure,
imagesc(squeeze(spikeTrigPhaseMap(46,:,:)));
map = colorcet( 'C2' );
map = circshift(map,1);
colormap(map)
c = colorbar;
