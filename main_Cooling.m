...
%% Making fake IntanBehaviour variable

IntanBehaviour.baseline(1).LFPIndex = [1:1:size(LFP.LFPdatacube,3)];
IntanBehaviour.baseline(1).LFPtime = LFP.times;
IntanBehaviour.baseline(1).rawLFP = LFP.LFPdatacube;
IntanBehaviour.baseline(1).xgp = LFP.xgp;
IntanBehaviour.baseline(1).xf = LFP.xf;
IntanBehaviour.baseline(1).wt = LFP.wt;


%% Power Spectrum during task across channels 
[PSD.ChBaseline , PSD.f] = getAvgPSD(IntanBehaviour.baseline,parameters);
PSD.trialPSDBaseline = squeeze(10*log10(mean(PSD.ChBaseline,2,"omitnan")));

PSD.avgPSDBaseline = squeeze(10*log10(mean(PSD.ChBaseline,[1 2],"omitnan")));

figure();
plot(PSD.f(1:81),PSD.trialPSDBaseline(1:81),'Color', [0 0 1 0.1]);
hold on;
plot(PSD.f(1:81),PSD.avgPSDBaseline(1:81),'Color', [0 0 1 1],'LineWidth',1.5);
ylim([0 50]);
xlabel('Frequency (Hz)');
ylabel('Power Spectral Density (dB/Hz)');
title('Average PSD for baseline');
box off;
...
plot(IntanBehaviour.cueHitTrace(1).time,smooth(PGD.avgPGDMiss,50,'sgolay',20),'-k','LineWidth',1);
