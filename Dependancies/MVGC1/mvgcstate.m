%% MVGC Analysis: state-space method.
...
% ------------------------------ Input ---------------------------------%
% dat: time series data organized as [node timestamps trials] (dimension 3)

%------------------------------ Output ---------------------------------%
% P:    granger causality coefficient across frequency
% F:    F-value statistic
% pval: P-value statistic
% ops:  meta data structure for modal parameters
% info: model output during fitting
%% References
%
...
% Modified by Hammad F. Khan (khan332@purdue.edu)
%% Toolbox download
% https://github.com/SacklerCentre/MVGC1

function output = mvgcstate(dat)
%% Parameters
...
fres      = 1/0.001;     % frequency resolution (empty for automatic calculation)

%% Check t-series data
% X structured as (node,samples,trials)
X = dat;
nvars = size(X,1);
nobs = size(X,2);
ntrials = size(X,3);
...
output.P = f;
output.sig = sig;
output.ops.regmode = regmode;  
output.ops.icregmode = icregmode;

output.ops.morder    = morder;
output.ops.momax     = momax;

output.ops.acmaxlags = acmaxlags;

output.ops.tstat     = tstat;
output.ops.alpha     = alpha;
output.ops.mhtc      = mhtc;

output.ops.fs        = fs; 
output.ops.fres      = fres;
