...
waveDensity = sum(waves1Present,1)/size(behaviourTrace1,2);
...
if waves2Flag == 1
    ...
    set(gca,'TickDir','out','fontsize',14'); box off;
    ax4 = subplot(4,1,4)
    waveDensity = sum(waves2Present,1)/size(behaviourTrace2,2);
    bar(waveDensity);hold on;
    [cur, ~] = fit((1:3001)',waveDensity','cubicinterp');
    b = feval(cur,1:3001);
    plot(smooth(b,100,'sgolay',4));
...
% wavesPresent = vertcat(Waves.wavesHit.wavePresent);
