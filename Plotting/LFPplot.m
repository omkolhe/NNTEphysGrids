...
subplot(4,1,1),plot(LFP.times(:,1:win),LFP.LFP(channel,start:stop),'k'); title('Raw Data'),box off,ylim([-300 300])
subplot(4,1,2),plot(LFP.times(:,1:win),LFP.theta_band(channel,start:stop),'r');title('\theta Band'),box off,ylim([-100 100])
subplot(4,1,3),plot(LFP.times(:,1:win),LFP.beta_band(channel,start:stop),'m');title('\beta Band'),box off,ylim([-100 100])
subplot(4,1,4),plot(LFP.times(:,1:win),LFP.gamma_band(channel,start:stop),'g');title('\gamma Band'),box off,ylim([-100 100])
